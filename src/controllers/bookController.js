...
const bookList = async function (req, res) {
  let selectedBooks = await BookModel.find().select({
    bookName: 1,
    authorName: 1,
    _id: 0,
  });
  res.send({ msg: selectedBooks });
};
//---------------------Third assignment----------------------------
const getBooksInYear = async function (req, res) {
  let reqYear = req.body.year;
  let thisYear = await bookModel.find({ year: reqYear });
  res.send({ msg: thisYear });
};
//---------------------Fourth assignment----------------------------
const getParticularBooks = async function (req, res) {
  let statement = req.body;
  const particularBook = await BookModel.find(statement);
...
const getXINRBooks = async function (req, res) {
  let inrPrice = await bookModel.find({ prices: { indianPrice: 1000 } });
  res.send({ msg: inrPrice });
};
//---------------------------sixth assignment--------------------
const getRandomBooks = async function (req, res) {
  let availabeBook = await bookModel
    .find({ stockAvailable: true })
    .find({ totalPages: { $gt: 500 } });

  res.send({ msg: availabeBook });
};
